...
program
    ...
    .option('-f, --force', 'Froces the creation.')
    .action(async (...options) => {
        ...
        if (
            (await CURRENT_WORKING_DIRECTORY.hasEntry(CONFIG_FILE_NAME)) &&
            !force
        )
            return console.log(ERROR_COLOR + COMMAND_INIT_CONFIG_EXISTS);
        ...
        const context = ProjectContext.CreateProject(
...
async function InitProject(context: ProjectContext) {
    ...
    const relativeDirectory =
        context.config.sourceFile.getBaseDirectory() ??
    ...
    if (config.getPacks()) {
        ...
        if (packs.behaviorPack) {
            ...
            const data = await relativeDirectory.createDirectory(
                packs.behaviorPack
            );
        ...
        if (packs.resourcePack) {
            ...
            await ProjectPack.Initialize(data, context, manifest, console.log);
        }
    }
}
async function ProccessQuestions(
    questions: Question[],
    ...
    for (const q of questions) {
        while (true) {
            ...
            if (moreQuestions)
                await ProccessQuestions(moreQuestions, reader, config, DATA);
