...
export class StringValidator extends BaseValidator{
    ...
    public validate(data: any, path: string[]): boolean {
        if(typeof data !== "string") throw new ValidationError("String expected, received '"+(typeof data)+"'", path);
        if(!this.validValues || this.validValues.length<=0) return true;
        if(!this.validValues.includes(this.caseCare?data:data.toLowerCase())) throw new ValidationError("Received string doesn't matches validValus, received '"+ data +"', expected: " + this.validValues.map(e=>`'${e}'`).join(", "), path);
        return true;
    }
...
export class NumberValidator extends BaseValidator{
    ...
    public constructor(){ super(); }
    public validate(data: any, path: string[]): boolean {
        if(typeof data !== "number") throw new ValidationError("Number expected, received '"+(typeof data)+"'", path);
        if(typeof this.max === "number" && data > this.max) throw new ValidationError(`The number exceeds the maximum value, '${data}' is more than '${this.max}'"`, path);
        if(typeof this.min === "number" && data < this.min) throw new ValidationError(`The number does not reach the minimum value, '${data}' is less than '${this.min}'"`, path);
        return true;
    }
    public rawObjectType(): any{
        return {
...
export class BooleanValidator extends BaseValidator{
    ...
    public constructor(){ super(); }
    public validate(data: any, path: string[]): boolean {
        if(typeof data !== "boolean") throw new ValidationError("Boolean expected, received '"+(typeof data)+"'", path);
        return true;
    }
    public rawObjectType(): any{ return { type: AbstractValueType.boolean }; } 
}
ValidatorLoader.RegisterType(AbstractValueType.boolean, (obj)=>{ return new BooleanValidator(); })