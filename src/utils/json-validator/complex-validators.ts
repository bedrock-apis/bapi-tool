    ...
    public validate(value: any, path: string[]): boolean {
        if (typeof value !== 'object')
            throw new ValidationError(
                "Object expected, received '" + typeof value + "'",
                path
            );
        for (const property of this.properties)
            if (!(property.propertyName in value) && !property.isOptional)
                throw new ValidationError(
                    "Missing object property '" + property.propertyName + "'",
                    path
                );
            else if (property.propertyName in value)
                return property.validate(value[property.propertyName], [
                    ...path,
...
ValidatorLoader.RegisterType(AbstractValueType.compoud, (obj) => {
    ...
    type.properties = obj.properties.map((e: any) =>
        ValidatorLoader.LoadType('__property', e)
    );
...
ValidatorLoader.RegisterType('__property', (obj) => {
    const type = new ObjectPropertyValidator(
        ValidatorLoader.LoadType(obj.type as string, obj),
        obj.name
    );
...
export class MapValidator extends BaseValidator {
    ...
    public validate(value: any, path: string[]): boolean {
        ...
        for (const property of Object.getOwnPropertyNames(value))
            if (!this.valueType.validate(value[property], [...path, property]))
                return false;
        return true;
    }
    public rawObjectType() {
...
export class ArrayValidator extends BaseValidator {
    ...
    public validate(value: any, path: string[]): boolean {
        if (!Array.isArray(value))
            throw new ValidationError(
                "Array expected, received '" + typeof value + "'",
                path
            );
        for (const [index, v] of value.entries())
            if (!this.valueType.validate(v, [...path, `[${index}]`]))
                return false;
        return true;
    }
    public rawObjectType() {
...
export class ComplexValidator extends BaseValidator {
    ...
    public validate(value: any, path: string[]): boolean {
        ...
        if (lastError)
            throw new ValidationError(
                `Type of ${typeof value} doesn't match any of these allowed type: ${this.validTypes.map((e) => e.name)}`,
                path
            );
        return true;
    }
...
ValidatorLoader.RegisterType(AbstractValueType.complex, (obj) => {
    ...
    for (const type of obj.validTypes)
        base.validTypes.push(ValidatorLoader.Load(type));
    return base;
});
