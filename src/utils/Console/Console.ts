...
export class Console{
    public CreateReader(): ConsoleReader{
        return new ConsoleReader(createInterface({input: process.stdin, output: process.stdout}));
    }
    private _write(text: string | Buffer): Promise<boolean>{
        return new Promise<boolean>((res)=>process.stdout.write(text,e=>res(Boolean(e))));
    }
    WriteLine(...params: any): Promise<boolean>{ return this._write(params.join() + "\r\n"); }
    Write(...params: any): Promise<boolean>{ return this._write(params.join()); }
...
export const console: Console & typeof globalThis["console"] = new Console() as any;