...
export class ProjectConfig {
     ...
     */
    async load() {
        if (!(await this.sourceFile.isValid()))
            throw new Error('Config file not found: ' + this.sourceFile.name);
    ...
    async save() {
        if (!this.isLoaded)
            throw new Error('Config must be loaded before saving.');
        try {
    ...
    getVersion() {
        return this.rawObject.version as string;
    }
    setVersion(data: string) {
        this.rawObject.version = data;
    }
    getName() {
        return this.rawObject.name as string;
    }
    setName(data: string) {
        this.rawObject.name = data;
    }
    getAuthor() {
        return this.rawObject.author as string;
    }
    setAuthor(data: string) {
        this.rawObject.author = data;
    }
    getPacks() {
        return this.rawObject.packs as any;
    }
    setPacks(data: any) {
    ...
    async *getPackDirectories(): AsyncIterableIterator<VirtualDirectory> {
        ...
        if (!(await dir.isValid()))
            throw new ReferenceError('Could not load config directory: ' + dir);
    ...
    async *getProjectPack(): AsyncIterableIterator<ProjectPack> {
        for await (const dir of this.getPackDirectories()) {
            ...
            if (manifestFile != null) {
                ...
                yield p.setManifestData(
                    JSON.parse((await manifestFile.readFile()).toString())
                );
            }
        }
    }
