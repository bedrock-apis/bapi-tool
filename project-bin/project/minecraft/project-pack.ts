...
export class ProjectPack {
    ...
    public static async Initialize(
        ...
        log(
            (
                await texts.createFile(
                    'languages.json',
                    JSON.stringify(['en_US'], null, '  ')
                )
            ).fullPath
        );
        log(
        ...
        if (Manifest.HasScripting(manifest)) {
            const file = await rootDirectory.createFile(
                manifest.modules[0].entry!,
        ...
        log(
            (
                await rootDirectory.createFile(
                    MANIFEST_FILE_NAME,
        ...
        log(
            (
                await rootDirectory.createFile(
                    PACK_ICON_FILE_NAME,
                    Buffer.from(PACK_ICON_PNG, 'base64')
                )
            ).fullPath
        );
    }
    public static async OpenPack(rootDirectory: VirtualDirectory) {
        if (!(await rootDirectory.hasFile(MANIFEST_FILE_NAME)))
            throw new ReferenceError('Manifest not found in ' + rootDirectory);
        const data = await rootDirectory
            .getFile(MANIFEST_FILE_NAME)
            .tryReadFile();
        if (data == null)
            throw new Error('Faild to read manifest file in ' + rootDirectory);
        const raw = parse(data.toString()) as unknown as Manifest;
        if (raw.format_version !== 2)
            throw new SyntaxError(
                'Unsuported manifest format: ' + raw.format_version
            );
     ...
     */
    public static async OpenEmpty(rootDir: VirtualDirectory) {
        return new ProjectPack(rootDir);
    }
    protected constructor(rootDirectory: VirtualDirectory) {
    ...
    public async isValid() {
        return (
            (await this.rootDir.isValid()) &&
            (await this.rootDir.hasFile('manifest.json'))
        );
    }
    public async saveManifest() {
        await this.rootDir
            .getFile(MANIFEST_FILE_NAME)
            .create(stringify(this.manifestData, null, '  '));
    }
    public setManifestData(data: any) {
        if (data.format_version !== 2)
            throw new SyntaxError(
                'Unsuported manifest format: ' + data.format_version
            );
